...
DROP TABLE IF EXISTS `t_schedule_job_group`;
CREATE TABLE `t_schedule_job_group`
(
    `job_group_id`   BIGINT(20)   NOT NULL COMMENT '主键ID',
    `schedule_name`  VARCHAR(200) NULL COMMENT '调度器名字',
    `job_group_name` VARCHAR(200) NULL COMMENT 'Job组',
    ...
    PRIMARY KEY (`job_group_id`) USING BTREE
) ENGINE = INNODB
  DEFAULT CHARSET = utf8 COMMENT 'JOB组实例';
...
(
    ...
    `job_cron_expression` VARCHAR(200) NULL COMMENT 'cron表达式',
    `create_time`   datetime(0)  NULL COMMENT '创建时间',
    `create_user`   VARCHAR(50)  NULL COMMENT '创建用户',
    `modify_time`   datetime(0)  NULL COMMENT '修改时间',
    `modify_user`   VARCHAR(50)  NULL COMMENT '修改用户',
  ...
  DEFAULT CHARSET = utf8 COMMENT '调度JOB实例';

DROP TABLE IF EXISTS `t_schedule_job_inst`;
CREATE TABLE `t_schedule_job_inst`
(
    `job_inst_id`              BIGINT(20)   NOT NULL COMMENT '主键ID',
    `schedule_name`            VARCHAR(200) NULL COMMENT '调度器名字',
    `job_name`                 VARCHAR(200) NULL COMMENT 'Job名字',
    `job_group_id`             VARCHAR(200) NULL COMMENT 'Job组',
    `description`              VARCHAR(200) NULL COMMENT '描述',
    `job_inst_class_name`      VARCHAR(200) NULL COMMENT 'JOB类路径',
    `job_inst_cron_expression` VARCHAR(200) NULL COMMENT 'cron表达式',
    `deleted`                  INT(2)       NULL COMMENT '是否逻辑删除 1：已删除 0：未删除',
    `create_time`              datetime(0)  NULL COMMENT '创建时间',
    `create_user`              VARCHAR(50)  NULL COMMENT '创建用户',
    `modify_time`              datetime(0)  NULL COMMENT '修改时间',
    `modify_user`              VARCHAR(50)  NULL COMMENT '修改用户',
    PRIMARY KEY (`job_inst_id`) USING BTREE
) ENGINE = INNODB
  DEFAULT CHARSET = utf8 COMMENT '调度JOB实例';
