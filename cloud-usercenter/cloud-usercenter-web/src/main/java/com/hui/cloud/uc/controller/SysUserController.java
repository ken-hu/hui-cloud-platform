...
import com.hui.cloud.auth.api.AuthApi;
...
public class SysUserController {
    ...
    @Autowired
    public SysUserController(SysUserService sysUserService, SysRoleService sysRoleService, SysGroupService sysGroupService, AuthApi authApi) {
    ...
    @PostMapping("/login")
    public ResponseVO login(@RequestBody LoginRequestDTO loginRequestDTO, HttpServletRequest request, HttpServletResponse response){
        // 校验token
        ResponseVO<AuthTokenDTO> responseVO = authApi.getToken(loginRequestDTO.getUserName(), loginRequestDTO.getPassword(), "password");
        if (responseVO.getCode().equals(ResponseVO.ResponseConstant.SUCCESS_CODE)) {
